...
const HERO_IMAGE = "/images/zaya-hero-dark.webp";
...
const aromas = [
  { name: "Bamboo", description: "Fresco, verde, elegante e sofisticado.", color: "#7e9e62", image: "bamboo", asset: "/images/aroma-bamboo.webp" },
  { name: "Roupa Limpa", description: "Cheiro de limpeza, conforto e roupa recém-lavada.", color: "#d7d7cf", image: "clean", bestSeller: true, asset: "/images/aroma-roupa-limpa.webp" },
  { name: "Flor de Algodão", description: "Delicado, suave, aconchegante e limpo.", color: "#bca89b", image: "cotton", bestSeller: true, asset: "/images/aroma-flor-algodao.webp" },
  { name: "Capim-Limão", description: "Cítrico, refrescante e energizante.", color: "#aebd62", image: "lemon", bestSeller: true, asset: "/images/aroma-capim-limao.webp" },
  { name: "Lavanda", description: "Suave, relaxante e clássico.", color: "#8b76a8", image: "lavender", asset: "/images/aroma-lavanda.webp" },
  { name: "Lavanda Francesa", description: "Floral, elegante e envolvente.", color: "#80669f", image: "lavender-two", premium: true, asset: "/images/aroma-lavanda-francesa.webp" },
  { name: "Figo Negro", description: "Intenso, sofisticado e marcante.", color: "#765a55", image: "fig", premium: true, asset: "/images/aroma-figo-negro.webp" },
  { name: "Mamãe e Bebê", description: "Suave, acolhedor e delicadamente confortável.", color: "#d8c2b2", image: "mamae-bebe", asset: "/images/aroma-mamae-bebe.webp" },
];

type CartItem = { id: string; name: string; volume: string; price: number; qty: number };
const money = (value: number) => value.toFixed(2).replace(".", ",");
const messageFor = (items: CartItem[], discount = 0) => `Olá, Zaya! Quero finalizar meu pedido:\n${items.map((item) => `${item.qty}x ${item.name} — ${item.volume} — R$ ${money(item.price * item.qty)}`).join("\n")}${discount ? `\nDesconto Oferta 3: - R$ ${money(discount)}` : ""}\n\nTotal: R$ ${money(items.reduce((sum, item) => sum + item.price * item.qty, 0) - discount)}`;
...
function AromaProductCard({ aroma, onAdd }: { aroma: (typeof aromas)[number]; onAdd: (items: { volume: string; price: number; qty: number }[]) => void }) {
  ...
  const imagePosition = aroma.image === "bamboo" ? "35% 40%" : aroma.image === "clean" ? "70% 70%" : aroma.image === "cotton" ? "90% 90%" : aroma.image === "lemon" ? "20% 50%" : aroma.image === "lavender" || aroma.image === "lavender-two" ? "20% 20%" : aroma.image === "fig" ? "85% 35%" : "center";
  return (
    <article className="dark-aroma-card">
      <div className={`dark-aroma-image aroma-image-${aroma.image}`}>
        {aroma.asset && <img src={aroma.asset} alt={`Aroma ${aroma.name} da Zaya Aromas Premium`} className="aroma-seo-image" width={800} height={800} loading="lazy" decoding="async" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: imagePosition }} />}
...
export default function Home() {
  ...
  const rawTotal = useMemo(() => cart.reduce((sum, item) => sum + item.price * item.qty, 0), [cart]);
  const oneLiterQty = cart.filter((item) => item.volume === "1 litro").reduce((sum, item) => sum + item.qty, 0);
  const discount = oneLiterQty >= 2 ? 9.9 : 0;
  ...
  const addToCart = (name: string, volume: string, price: number, quantity = 1) => {
    setCart((current) => { const id = `${name}-${volume}`; const found = current.find((item) => item.id === id); if (found) return current.map((item) => item.id === id ? { ...item, qty: item.qty + quantity } : item); return [...current, { id, name, volume, price, qty: quantity }]; });
    setToast("Produto adicionado ao carrinho"); window.setTimeout(() => setToast(""), 3000);
  };
  ...
  const checkoutHref = `${WHATSAPP_BASE}${encodeURIComponent(cart.length ? messageFor(cart, discount) : "Olá, Zaya! Quero conhecer os aromas e fazer um pedido.")}`;
  ...
  return (
    <div className="zaya-page dark-store">
      ...
      <Footer checkoutHref={checkoutHref} />
